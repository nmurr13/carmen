...
SELECT code, name FROM country WHERE region ='Southern Europe' ORDER BY population ASC LIMIT 1;
...
SELECT language, isofficial FROM countrylanguage WHERE 'VAT' = countrycode;
...
SELECT * FROM countrylanguage WHERE 'Italian' = language ORDER BY percentage DESC LIMIT 1;

-- countrycode	language	isofficial	percentage
-- 	SMR	Italian	t	100

SELECT name,code,continent,region FROM country WHERE code ='SMR';

-- name	code	continent	region
-- San Marino	SMR	Europe	Southern Europe
...
SELECT name,id FROM city WHERE countrycode = 'SMR';
...
-- different parts of the globe! She's headed to South America as we speak; 
...
SELECT name,id,countrycode FROM city WHERE name LIKE 'Serra%';

-- name	id	countrycode
-- 1	Serra	265	BRA
-- 2	Serravalle	3170	SMR

SELECT name,continent,region FROM country WHERE code='BRA';

-- #	name	continent	region
-- 1	Brazil	South America	South America
...
-- She's in Santa Monica, CA in the USA!
